      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '24px' }}>
        ...
        {tab === 'overview' && data && (
          <>
            ...
            {data.total === 0 && (
              <div style={{ background: 'white', borderRadius: '14px', padding: '40px', textAlign: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                <div style={{ fontSize: '48px', marginBottom: '16px' }}>📊</div>
                <h3 style={{ color: '#0c4a6e', margin: '0 0 8px' }}>Aucune analyse pour l'instant</h3>
                <p style={{ color: '#64748b', fontSize: '14px' }}>Les données apparaîtront ici dès la première analyse effectuée.</p>
              </div>
            )}
          </>
        )}
                  ...
                  <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', marginBottom: '20px' }}>
                    ...
                    <div>
                      <p style={{ margin: '0 0 4px', fontSize: '13px', color: '#64748b' }}>
                        📅 {new Date(selectedAnalysis.created_at).toLocaleString('fr-FR')}
                      </p>
                      {selectedAnalysis.location_data?.city && (
                        <p style={{ margin: '0 0 4px', fontSize: '13px', color: '#64748b' }}>
                          📍 {selectedAnalysis.location_data.city}, {selectedAnalysis.location_data.country}
                        </p>
                      )}
                      <p style={{ margin: '0 0 4px', fontSize: '13px', color: '#64748b' }}>
                        💧 {selectedAnalysis.pool_volume}m³ — {selectedAnalysis.photo_count} photo{selectedAnalysis.photo_count > 1 ? 's' : ''}
                      </p>
                    </div>
                  </div>
                        ...
                        {selectedAnalysis.photo_urls.map((url: string, i: number) => (
                          <a key={i} href={url} target="_blank" rel="noopener noreferrer">
                            <img src={url} alt={`Photo ${i + 1}`}
                              style={{ width: '120px', height: '120px', objectFit: 'cover', borderRadius: '10px', border: '2px solid #e2e8f0' }} />
                          </a>
                        ))}
                      </div>
                    </div>
                      ...
                      <div key={i} onClick={() => setSelectedAnalysis(a)}
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
                            {a.location_data?.city && (
                              <span style={{ fontSize: '12px', color: '#94a3b8' }}>📍 {a.location_data.city}</span>
                            )}
